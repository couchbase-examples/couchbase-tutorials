...
alt_paths:
  - "/tutorial-azure-openai-couchbase-rag-with-hyperscale-vector-index"
  - "/tutorial-azure-openai-couchbase-rag-with-composite-vector-index"
title: RAG with Azure OpenAI and Couchbase Hyperscale and Composite Vector Index
short_title: RAG with Azure OpenAI & Hyperscale/Composite Index
description:
  - Learn how to build a semantic search engine using Couchbase and Azure OpenAI using Hyperscale and Composite Vector Index.
...
[View Source](https://github.com/couchbase-examples/vector-search-cookbook/tree/main/azure/query_based/RAG_with_Couchbase_and_AzureOpenAI.ipynb)

## Introduction
In this guide, we will walk you through building a powerful semantic search engine using Couchbase as the backend database, [Azure OpenAI](https://azure.microsoft.com/) as the AI-powered embedding and language model provider. Semantic search goes beyond simple keyword matching by understanding the context and meaning behind the words in a query, making it an essential tool for applications that require intelligent information retrieval. This tutorial is designed to be beginner-friendly, with clear, step-by-step instructions that will equip you with the knowledge to create a fully functional semantic search system using Hyperscale and Composite Vector Index from scratch. Alternatively if you want to perform semantic search using the Search Vector Index, please take a look at [this.](https://developer.couchbase.com/tutorial-azure-openai-couchbase-rag-with-search-vector-index/)

## How to run this tutorial

This tutorial is available as a Jupyter Notebook (`.ipynb` file) that you can run interactively. You can access the original notebook [here](https://github.com/couchbase-examples/vector-search-cookbook/blob/main/azure/query_based/RAG_with_Couchbase_and_AzureOpenAI.ipynb).
...
```python
!pip install --quiet datasets==3.5.0 langchain-couchbase==0.5.0 langchain-openai==0.3.32 python-dotenv
```
...
```
    ...
    2026-04-06 11:17:22,958 - INFO - Successfully connected to Couchbase
...
```
    ...
    2026-04-06 11:17:32,968 - WARNING - Warning: You are sending unauthenticated requests to the HF Hub. Please set a HF_TOKEN to enable higher rate limits and faster downloads.
    2026-04-06 11:17:37,760 - INFO - Successfully loaded the BBC News dataset with 2687 rows.
...
```
    ...
    2026-04-06 11:18:12,436 - INFO - Successfully created vector store
...
```
    ...
    2026-04-06 11:23:35,276 - INFO - Document ingestion completed successfully.
...
```
    ...
    2026-04-06 11:25:53,986 - INFO - Successfully created Azure OpenAI Chat model and verified deployment
...
# Performance timing variables used in the summary section
baseline_search_time = None
indexed_search_time = None
...
try:
    ...
    baseline_search_time = search_elapsed_time
...
```
    ...
    2026-04-06 11:25:57,310 - INFO - Semantic search completed in 1.88 seconds


    
    Semantic Search Results (completed in 1.88 seconds):
    ...
    Distance: 0.4598, Text: Second seed Smith knocked out of Worlds by Doets
...
In the code below, we demonstrate creating a Hyperscale index. This method takes an index type (BHIVE or COMPOSITE) and description parameter for optimization settings. Alternatively, Hyperscale and Composite Vector Indexes can be created manually from the Couchbase UI.
...
Note: In Hyperscale and Composite Vector Index search, the distance represents the vector distance between the query and document embeddings. Lower distance indicate higher similarity, while higher distance indicate lower similarity.
...
def perform_indexed_search():
    """Wrapper function for indexed semantic search to enable retry logic."""
    start_time = time.time()
    results = vector_store.similarity_search_with_score(query, k=10)
    elapsed = time.time() - start_time
    return results, elapsed
...
try:
    # Perform the semantic search with retry logic
    search_results, search_elapsed_time = retry_with_backoff(perform_indexed_search)
    indexed_search_time = search_elapsed_time
...
```
    ...
    2026-04-06 11:26:14,021 - INFO - Semantic search completed in 1.08 seconds


    
    Semantic Search Results (completed in 1.08 seconds):
    --------------------------------------------------------------------------------
    Distance: 0.3697, Text: The Littler effect - how darts hit the bullseye
...
Choose based on your specific use case and query patterns. For this tutorial's question-answering scenario using the BBC News dataset, either index type would work, but Hyperscale might be more efficient for pure semantic search across questions.
...
```
    ...
    2026-04-06 11:26:25,840 - INFO - Successfully created cache
...
```
    ...
    2026-04-06 11:26:27,229 - INFO - Successfully created RAG chain
...
def perform_rag():
    """Wrapper function for RAG chain to enable retry logic."""
    return rag_chain.invoke(query)

try:
    rag_response = retry_with_backoff(perform_rag)
    rag_elapsed_time = time.time() - start_time
    print(f"RAG Response: {rag_response}")
    print(f"RAG response generated in {rag_elapsed_time:.2f} seconds")
except InternalServerFailureException as e:
    if "query request rejected" in str(e):
        print("Error: Search request was rate limited after multiple retries. Please wait and try again later.")
    else:
...
```
    ...
    1. **Tournament Record Average**: Littler hit a tournament record average of 140.91 during his match.
    2. **Match Victory**: He won his second-round match against Ryan Meikle with a score of 3-1.
    3. **Near Nine-Darter**: He was close to achieving a nine-darter but missed double 12 while attempting it.
    4. **Outstanding Fourth Set Performance**: In the fourth set, he hit four maximum 180s and clinched three straight legs with impressive darts.
    5. **Overall Match Average**: He finished the match with a significant overall average of 100.85.
    6. **Emotional Reaction**: Littler expressed his emotions after the match, stating it was one of the toughest games he had played and was overcome with emotion during his on-stage interview.
    
    These achievements highlight Littler's impressive performance as he aspires to compete for the PDC World Championship title.
    RAG response generated in 4.80 seconds
...
try:
    ...
    for i, query in enumerate(queries, 1):
        ...
        def perform_query():
            """Wrapper for query execution with retry logic."""
            return rag_chain.invoke(query)
        ...
        # Track cache timings from repeated query pattern
        if i == 1:
            cache_miss_time = elapsed_time
        elif i == 3:
            cache_hit_time = elapsed_time
...
```
    ...
    Response: In the match between Fulham and Liverpool, which ended in a 2-2 draw, Liverpool faced a challenging situation after Andy Robertson received a red card in the 17th minute, leaving the team with 10 players for the majority of the match. Despite being at a numerical disadvantage, Liverpool twice came from behind to equalize, with Diogo Jota scoring the crucial equalizer in the 86th minute. Fulham had a strong performance, which was acknowledged by their player Antonee Robinson, who noted that Liverpool did not play like a team with 10 men. Liverpool maintained over 60% possession and led in many attacking metrics. Liverpool’s head coach, Arne Slot, praised his team's spirit and character despite the red card and setbacks they faced during the match. The result allowed Liverpool to extend their lead at the top of the Premier League, while their title rivals Arsenal failed to capitalize on the opportunity.
...
Let's summarize the performance improvements achieved through Hyperscale Vector Index and caching:
...
print("=" * 80)
print(f"{'Phase 1 - Baseline Search (No Hyperscale):':<45} {baseline_search_time:.4f} seconds")
print(f"{'Phase 2 - Hyperscale-Optimized Search:':<45} {indexed_search_time:.4f} seconds")
print("Phase 3 - Cache Benefits:")
print(f"  {'First execution (cache miss):':<40} {cache_miss_time:.4f} seconds")
print(f"  {'Second execution (cache hit):':<40} {cache_hit_time:.4f} seconds")
print()
print("-" * 80)
print("VECTOR SEARCH OPTIMIZATION IMPACT:")
print("-" * 80)

if baseline_search_time and indexed_search_time:
    hyperscale_speedup = baseline_search_time / indexed_search_time
    hyperscale_improvement = ((baseline_search_time - indexed_search_time) / baseline_search_time) * 100
    print(f"{'Hyperscale Index Benefit:':<30} {hyperscale_speedup:.2f}x faster ({hyperscale_improvement:.1f}% improvement)")
else:
    print(f"{'Hyperscale Index Benefit:':<30} N/A")

if cache_miss_time and cache_hit_time:
    cache_speedup = cache_miss_time / cache_hit_time
    cache_improvement = ((cache_miss_time - cache_hit_time) / cache_miss_time) * 100
    print(f"{'Cache Benefit:':<30} {cache_speedup:.2f}x faster ({cache_improvement:.1f}% improvement)")
else:
    print(f"{'Cache Benefit:':<30} N/A")
```

    ================================================================================
    VECTOR SEARCH PERFORMANCE OPTIMIZATION SUMMARY
    ================================================================================
    Phase 1 - Baseline Search (No Hyperscale):    1.8819 seconds
    Phase 2 - Hyperscale-Optimized Search:        1.0840 seconds
    Phase 3 - Cache Benefits:
      First execution (cache miss):            3.6641 seconds
      Second execution (cache hit):            0.3782 seconds
    
    --------------------------------------------------------------------------------
    VECTOR SEARCH OPTIMIZATION IMPACT:
    --------------------------------------------------------------------------------
    Hyperscale Index Benefit:      1.74x faster (42.4% improvement)
    Cache Benefit:                 9.69x faster (89.7% improvement)


## Conclusion

You've built a semantic search engine using Couchbase Hyperscale and Composite Vector Index with Azure OpenAI embeddings and LangChain. For the Search Vector Index alternative, see the [search_based tutorial](https://developer.couchbase.com/tutorial-azure-openai-couchbase-rag-with-search-vector-index).
