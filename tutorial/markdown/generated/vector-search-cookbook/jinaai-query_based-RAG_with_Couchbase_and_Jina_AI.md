...
[View Source](https://github.com/couchbase-examples/vector-search-cookbook/tree/main/jinaai/query_based/RAG_with_Couchbase_and_Jina_AI.ipynb)
...
3. **Enable Query Service:** Required for vector search functionality using Hyperscale and Composite vector index

## Setup and Installation

### Install Required Libraries

Install the necessary packages for Couchbase vector search, Jina AI integration, and LangChain RAG capabilities.
...
### Create Couchbase Vector Store

Set up the Couchbase vector store which enables both Hyperscale and Composite Vector Indexes for high-performance vector storage and similarity search using Couchbase's Query Service.
...
try:
    ...
    raise ValueError(f"Failed to create Couchbase vector store: {str(e)}")
```
...
**Important**: Hyperscale and Composite Vector Indexes must be created AFTER uploading vector data. The index creation process analyzes existing vectors to optimize search performance through clustering and quantization.
...
3. **Cache Benefits**: Show how caching can be applied on top of Hyperscale/Composite Vector Index for repeated queries
...
For this tutorial, we'll use **Hyperscale Vector Indexes:** as it's optimized for pure semantic search scenarios.
...
For more information on Hyperscale and Composite vector indexes, see [Couchbase Hyperscale and Composite Vector Index Documentation](https://docs.couchbase.com/cloud/vector-index/use-vector-indexes.html).
...
### Test 1: Baseline Performance (No Hyperscale/Composite Vector Index)

Test pure vector search performance without Hyperscale/Composite Vector Index optimization.
...
### Test 2: Hyperscale vector indexes Optimized Performance

Test the same vector search with Hyperscale optimization.


```python
# Test vector search performance with Hyperscale index
...
hyperscale_time = test_vector_search_performance(vector_store, hyperscale_test_query, "Hyperscale-Optimized Search")
```
...
Initialize Jina's chat model for generating intelligent responses based on our Hyperscale/Composite optimized retrieval system.
...
Create the complete RAG pipeline that integrates our Hyperscale/Composite optimized vector search with Jina's language model.
...
Test the complete RAG system leveraging our Hyperscale/Composite performance optimizations.
