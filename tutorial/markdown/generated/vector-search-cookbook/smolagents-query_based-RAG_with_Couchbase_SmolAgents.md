...
alt_paths: ["/tutorial-smolagents-couchbase-rag-with-hyperscale-vector-index", "/tutorial-smolagents-couchbase-rag-with-composite-vector-index"]
title: RAG with Couchbase and smolagents using Couchbase Hyperscale and Composite Vector Index
short_title: RAG with smolagent using Couchbase Hyperscale and Composite Vector Index
description:
  - Learn how to build a semantic search engine using Couchbase and Hugging Face smolagents using Couchbase Hyperscale and Composite Vector Index.
  - This tutorial demonstrates how to integrate Couchbase's vector search capabilities with smolagents using Couchbase Hyperscale and Composite Vector Indexes.
...
[View Source](https://github.com/couchbase-examples/vector-search-cookbook/tree/main/smolagents/query_based/RAG_with_Couchbase_SmolAgents.ipynb)
...
This tutorial is available as a Jupyter Notebook (`.ipynb` file) that you can run interactively. You can access the original notebook [here](https://github.com/couchbase-examples/vector-search-cookbook/blob/main/smolagents/query_based/RAG_with_Couchbase_SmolAgents.ipynb).
...
## Understanding Hyperscale and Composite Vector Index
...
#### Hyperscale and Composite Vector Index vs Search Vector Index: Choosing the Right Approach
...
| **Filtering**         | Pre-filtering with `WHERE` clauses (Composite) or post-filtering  | Pre-filtering with flexible ordering |
...
#### Hyperscale and Composite Vector Index Types
...
- **Features**: 
  ...
  - Best for well-defined workloads requiring complex filtering using Hyperscale index features
...
#### Understanding Hyperscale and Composite Vector Index Configuration (Couchbase 8.0 Feature)
...
For more information on Hyperscale and Composite Vector Index, see [Couchbase Hyperscale and Composite Vector Index Documentation](https://docs.couchbase.com/cloud/vector-index/use-vector-indexes.html).
...
1. **Phase 1 - Baseline Performance**: Test vector search without Hyperscale vector index to establish baseline metrics
2. **Phase 2 - Hyperscale-Optimized Search**: Create Hyperscale index and measure performance improvements

**Important Context:**
- Hyperscale vector index performance benefits scale with dataset size and concurrent load
- With our dataset (~1,700 articles), improvements may be modest
- Production environments with millions of vectors show significant Hyperscale vector index advantages
- The combination of Hyperscale vector index + LLM caching provides optimal RAG performance
...
print("-" * 80)
print(f"{'Phase 1 - Baseline (No Hyperscale)':<35} {baseline_time:.4f}{'':16} ⚪ Baseline")
print(f"{'Phase 2 - Hyperscale-Optimized Search':<35} {hyperscale_time:.4f}{'':16} ✅ Optimized")
...
if baseline_time > hyperscale_time:
    ...
    print(f"\n⚠️  Note: Hyperscale Index was {slowdown_pct:.1f}% slower than baseline in this run")
...
print("KEY INSIGHTS:")
print("-"*80)
print("1. 🚀 Hyperscale Index Optimization:")
...
print("\n3. 🎯 When to Use Hyperscale Index:")
...
```
    ...
    Phase 2 - Hyperscale-Optimized     0.4157                 ✅ Optimized
    ...
    KEY INSIGHTS:
    --------------------------------------------------------------------------------
    1. 🚀 Hperscale Index Optimization:
    ...
    3. 🎯 When to Use Hyperscale Indexes:
